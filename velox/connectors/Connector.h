...
class Connector {
 public:
  ...
  static std::shared_ptr<ScanTracker> getTracker(const std::string& scanId);

 private:
  static void unregisterTracker(ScanTracker* tracker);

  const std::string id_;

  static std::mutex trackerMutex_;
  static std::unordered_map<std::string_view, std::weak_ptr<ScanTracker>>
      trackers_;
};
