...
struct ExecutableFragment {
  ...
  std::vector<LeafSplit> leafSplits;
...
/// Describes options for generating an executable plan. 
 struct ExecutablePlanOptions {
   // Maximum Number of independent Tasks for one stage of execution. If 1, there are no exchanges.
   int32_t numWorkers

   // Number of threads in a fragment in a worker. If 1, there are no local exchanges.
   int32_t numDrivers;
