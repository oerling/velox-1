...
struct BlockStatus {
  ...
  /// the BlockStatus array. First are grid level statuses for instructions that return a status. After this are block level statuses.

struct InstructionStatus {
  // Offset of containing instruction's grid state from the end of BlockStatus array.
  uint16_t gridState{0};
  // Total size of gridStates. Block level states start after the last grid state.
  uint16_t gridStateSize{0};
  // Start of per-block status. gridStateSize + gridDim.x * blocksPerThread * blockState' is the  offset of the first per block status from the end of BlockStatus array.
