...
inline __device__ int32_t nonNullIndex256(
    ...
    NonNullState* state) {
  if (threadIdx.x == 0) {
    state->nonNullsBelow += countBits(reinterpret_cast<uint64_t*>(nulls), state->nonNullsBelowRow, bitOffset);
  ...
  if (threadIdx.x < kWarpThreads) {
    ...
    uint32_t count =
        threadIdx.x < (blockDim.x / kWarpThreads) ? state->temp[threadIdx.x] : 0;
    uint32_t start;
...
inline __device__ int32_t nonNullIndex256Sparse(
  ...
  if (threadIdx.x < numRows) {
    ...
    int32_t previousRow = threadIdx.x == 0 ? state->nonNullsBelowRow : rows[threadIdx.x - 1] + 1;
  ...
  return state->temp[threadIdx.x / kWarpThreads] + previousOffset + nonNullsBelow - 1;
}
