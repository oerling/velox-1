...
class BlockTestStream : public Stream {
 public:
  ...
  /// Tests nonNullIndex256 if 'rows' are all consecutive integers and nonNullIndex256Sparse otherwise. If the row hits a null in 'nulls' the result in indices is -1, therwise it is the index in non null values, i.e. the original minus the count of null positions below it.
  void nonNullIndex(char* nulls, int32_t* rows, int32_t numRows, int32_t* indices, int32_t* temp);
};
