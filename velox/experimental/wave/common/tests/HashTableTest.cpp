...
"}\n";
  ...
  void updateJitMtxCoa(TestingRow* rows, HashRun& run, TestingRow* reference) {
    ...
    KernelSpec spec = {jitMtxCoa, {"facebook::velox::wave::jitSumMtxCoalesce"}, "sum1Jit.cu", 0, nullptr, nullptr};
    auto kernel = CompiledModule::create(spec);
    uint64_t micros = 0;
    {
    MicrosecondTimer t(&micros);                           
    void* params[] = {&rows, &run.probe};
    kernel->launch(0, run.numBlocks, run.blockSize, 0, streams_[0].get(), params);
    streams_[0]->wait();                                   
  }                                                        
  run.addScore("sum1JitMtxCoa", micros);                             
  compareAndReset(                                         
      reference, rows, run.numDistinct, "sum1JitMtxCoa", true, 1);
  }
