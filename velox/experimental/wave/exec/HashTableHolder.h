...
class HashTableHolder : public exec::BaseHashTable {
 public:
  ...
  virtual void joinProbe(HashLookup& lookup) override {
    VELOX_UNREACHABLE();
  }
  ...
  virtual int64_t allocatedBytes() const override {
    VELOX_UNREACHABLE();
  }

  virtual void clear(bool freeTable) override {
    VELOX_UNREACHABLE();
  }
  ...
  virtual uint64_t numDistinct() const override {
    VELOX_UNREACHABLE();
  }
  virtual HashTableStats stats() const override {
    VELOX_UNREACHABLE();
  }
  virtual uint64_t hashTableSizeIncrease(
      int32_t numNewDistinct) const override {
    VELOX_UNREACHABLE();
  }
  virtual uint64_t estimateHashTableSize(uint64_t numDistinct) const override {
    VELOX_UNREACHABLE();
  }

  virtual bool hasDuplicateKeys() const override {
    VELOX_UNREACHABLE();
  }
  virtual HashMode hashMode() const {
    VELOX_UNREACHABLE();
  }
  ...
  virtual void erase(folly::Range<char**> rows) override {
    VELOX_UNREACHABLE();
  }
