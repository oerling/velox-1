...
AdvanceResult Program::canAdvance(
  ...
  void Program::callUpdateStatus(WaveStream& stream, const std::vector<WaveStream*>& otherStreams, AdvanceResult& advance) {
  if (advance.updateStatus) {
    advance.updateStatus(stream, otherStreams, *instructions_[advance.instructionIdx]);
  }
}
