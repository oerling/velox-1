...
bool emptyLevel(std::vector<KernelBox> level) {
  ...
  // Sets 'op's output operand ids to be the outputs generated by steps from 'begin' to 'end'.
  void PipelineCandidate::setOutputIds(CompileState* state, WaveOperator* op, int32_t begin, int32_t end) {
  ...
  for (pipelineIdx_ = 0; pipelineIdx_ < selectedPipelines_.size();
    ...
    operators_.push_back(std::make_unique<Project>(
        *this,
        ...
        std::move(levels),
        nullptr));
    currentCandidate_->setOutputIds(this, operators_.back().get(), start, currentCandidate_->steps.size());
  }
}
