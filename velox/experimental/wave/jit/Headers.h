...
const char* velox_experimental_wave_exec_Accumulators_cuh =
    ...
    "void __device__ __forceinline__ simpleAccumulate(uint32_t peers, int32_t leader, int32_t lane, AccType* acc, uint32_t* aggNulls, uint32_t aggNullMask, IncType input, bool inputIsNull, Reduce2 reduce) {\n"
    "  IncType agg;\n"
    "  auto toUpdate = peers;\n"
    "  bool isAny = false;\n"
    "  for (;;) {\n"
    "    auto peer = __ffs(toUpdate) - 1;\n"
    "    auto inc = __shfl_sync(peers, peer, input);\n"
    "    auto isNull = __shfl_sync(peers, peer, inputIsNull);\n"
    "    if (lane == leader) {\n"
    "      if (!isNull) { \n"
