...
void DecodeRandomAccessLane(
  ...
  switch (op->nullMode) {
    ...
    case NullMode::kDenseNullable:
      dataIdx =
          nonNullIndex(op->nulls + (base / 8), numRows, nonNullOffset, temp);
      break;
    case NullMode::kSparseNullable:
      dataIdx = nonNullIndex256Sparse(
          op->nulls, op->rows, base, base + numRows, nonNullOffset, temp);
      break;
  }
  T value();
    ...
    switch (op->filterKind) {
      ...
      case kRange:
        if (dataIdx == -1) {
          ...
          filterPass =
              value >= op->filterLow<T>() && value <= op->filterHigh<T>();
        }
        break;
    }
  }
