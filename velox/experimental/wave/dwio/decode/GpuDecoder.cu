...
int32_t GpuDecode::sharedMemorySize() {
  ...
  /// Describes multiple sequences of decode ops. Each TB executes a sequence of decode steps. The data area starts with a range of instruction numbers for each thread block. The first TB runs from 0 to ends[0]. The nth runs from ends[nth-1] to ends[nth]. After gridDim.x ends, we round to an 8 aligned offset and have an array of GpuDecodes.]
  struct GpuDecodeParams {
    // If need to represent more than this many ops, use a dynamically allocated external array in 'external'.
    static constexpr int32_t kMaxInlineOps = (sizeof(GpuDecodeParams) / (sizeof(GpuDecode) + sizeof(int32_t))) - 1;

    // Pointer to standalone description of work. If nullptr, the description of work fits inline in 'this'.
    GpuDecodeParams* external{nullptr};
    // The end of each decode program. The first starts at 0. The end is ends[blockIdx.x].
    int32_t ends[100 * sizeof(GpuDecode) / sizeof(int32_t)] = {};
  };

  __global__ void decodeKernel(GpuDecodeParams inlineParams) {
    GpuDecodeParams* params = inlineParams.external ? inlineParams.external : &inlineParams;
    int32_t programStart = blockIdx.x == 0 ? 0 : params.ends[blockIdx.x - 1];
    int32_t programEnd = params.ends[blockIdx.x];
    GpuDecode* ops = reinterpret_cast<GpuDecode*>(&&params.starts[0] + roundUp(gridDim.x, 2));
    for (i = programStart; i < programEnd; ++i) {
      decodeSwitch<kBlockSize>(ops + i);
    }
  }

  void callGpuDecode(const std::vector<std::vector<GpuDecode>> programs, GpuArena* arena, WaveBufferPtr& extra, Stream* stream) { 
    ...
    if (numOps > GpuDecodeParams::kMaxInline) {
      extra = arena->allocate<char>((numBlocks + 1) * (sizeof(GpuDecode) + sizeof(int32_t)));
      params = extra->as<GpuDecodeParams>();
    }
    int32_t end = programs[0].size();
    GpuDecode* decodes = reinterpret_cast<GpuDecode*>(&params->ends[0] + roundUp(numBlocks, 2));
    int32_t fill = 0;
