...
BufferPtr zeroOutUnselectedRows(
  ...
  auto newIndices = allocateIndices(rows.size(), pool);
...
// Extend nulls buffer to rows.size().
BufferPtr extendNullsBuffer(
    BufferPtr& nulls,
    const SelectivityVector& rows,
    velox::memory::MemoryPool* pool) {
  if (!nulls->isView()) {
    AlignedBuffer::reallocate<bool>(&nulls, rows.size(), bits::kNull);
    return nulls;
  }
  auto newNulls = allocateNulls(rows.size(), pool);
  auto* rawNewNulls = newNulls->asMutable<uint64_t>();
  rows.applyToSelected([&](auto row) { bits::setNull(rawNewNulls, row); });
  return newNulls;
}
} // namespace
  ...
  if (wrapEncoding_ == VectorEncoding::Simple::DICTIONARY) {
    if (!peeledResult) {
      ...
      if (!rows.isAllSelected()) {
        ...
        if (nulls) {
          nulls = extendNullsBuffer(nulls, rows, pool);
        }
      }
