...
DEFINE_int32(min_partitioned_output_buffer_mb, 32, "Minimum MB of buffer for Task output");
DEFINE_int32(partitioned_output_continue_pct, 90, "Fill percentage of Task partitioned output for resuming a blocked Task");
...
void releaseAfterAcknowledge(
  ...
  PartitionedOutputBuffer::PartitionedOutputBuffer(
      ...
      : task_(std::move(task)),
        ...
        maxSize_(std::max<int64_t>(FLAGS_min_partitioned_output_buffer_mb << 20, task_->queryCtx()->config().maxPartitionedOutputBufferSize())),
