...
int64_t RowContainer::sizeIncrement(vector_size_t numRows, int64_t flatBytes) const {
  constexpr int32_t kAllocUnit = 64 << 10; // 64K.
  int32_t needRows = std::max<int64_t>(0, numRows - numFreeRows_);
  int64_t needBytes = stringAllocator_.cumulativeBytes()? std::min<int64_t>(0, stringAllocator_.freeSpace() - flatBytes) : 0;
