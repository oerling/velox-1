...
class LocalRunnerTestBase : public HiveConnectorTestBase {
 protected:
  ...
  /// Returns a split source factory that contains splits for the table scans in 'plan'. 'plan' should refer to testing tables created by 'this'.
  std::shared_ptr<TestingSplitSourceFactory> makeTestingSplitSourceFactory(const MultiFragmentPlanPtr& plan);
