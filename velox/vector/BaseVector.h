...
class BaseVector {
 public:
  ...
  /// partly filled and 'indices' undefined. Returns true if all
  /// elements of 'vector' are in 'alphabet' and their indices are set
  /// in 'indices'. 'map' is a map from the value to its index in 'alphabet'. If
  /// it is non-empty at time of call, it must correspond to 'alphabet'.
  static bool dictionarize(
  ...
  static VectorPtr appendWithEncoding(
      const VectorPtr& first,
      const VectorPtr& second);
