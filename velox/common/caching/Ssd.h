...
// A 64 bit word describing a SSD cache entry. 32 files x 64G per file, up to 8MB in entry size for a total of 2TB. Larger capacities need more bits.
class SsdRun {
 public:
 SsdRun() : _bits(0) {}
...
// Represents an SsdFile entry that is planned for load or being loaded. This is destroyed after load.
 class SsdPin {
 public:
...
class SsdFile {
 public:
  ...
  static constexpr kNumRegions = kMaxSize / kRegionSize;
  
  // Constructs a cache backed by filename. Discards any previous
  // contents of filename.
  SsdFile(std::string_view filename);

  // Adds entries of  'pins'  to this file. 'pins' must be in read mode and those pins that are successfully added to SSD are marked as being on SSD. The file of the entries must be a file that is backed by 'this'.
 ...
 private:
  ...
  // Count of reads and writes. The scores are decayed every time e count goes over kDecayInterval or half 'entries_' size, wichever comes first.
