...
struct SsdCacheStats {
  ...
  void operator =(const SsdCacheStats& other) {
    entriesWritten = atomicValue(other.entriesWritten);
    bytesWritten = atomicValue(other.bytesWritten);
    entriesRead = atomicValue(other.entriesRead);
    bytesRead = atomicValue(other.bytesRead);
    entriesCached = atomicValue(other.entriesCached);
    bytesCached = atomicValue(other.bytesCached);
    numPins = atomicValue(other.numPins);
  }

  asan_atomic<uint64_t> entriesWritten{0};
  asan_atomic<uint64_t> bytesWritten{0};
  asan_atomic<uint64_t> entriesRead{0};
  asan_atomic<uint64_t> bytesRead{0};
  asan_atomic<uint64_t> entriesCached{0};
  asan_atomic<uint64_t> bytesCached{0};
  asan_atomic<int32_t> numPins{0};
};
