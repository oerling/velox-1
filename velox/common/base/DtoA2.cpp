...
class FDBigInteger {
    ...
    static constexpr int SMALL_5_POW[] = {
            1,
            5,
            5 * 5,
            5 * 5 * 5,
    ...
    // Maximum size of cache of powers of 5 as FDBigIntegers.
     static constexpr int MAX_FIVE_POW = 340;
  ...
  static constexpr  long LONG_MASK = 0xffffffffL;


  std::vector<int> data;  // value: data[0] is least significant

  int offset = 0;  // number of least significant zero padding ints

     int nWords = 0;  // data[nWords-1]!=0, all values above are zero
  ...
  bool  isImmutable = false;
    ...
    /*@
     ...
     @*/
			   private FDBigInteger(std::vector<int> data, int offset) {
        this->data = data;
        this->offset = offset;
