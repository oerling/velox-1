...
/// Each query execution creates a dedicated memory pool object. The
/// memory pool objects from a query are organized as a tree with four levels
/// which mirror the query's physical execution plan:
///
...
/// by the first operator instantiated for the correspponding plan node. It is owned by Task via 'childPools_'
///
/// The bottom level consists of per-operator pools. These are children of the node pool that corresponds to the plan node from which the operator is created.
/// Operator and node pools are owned by the Task via 'childPools_'.
///
...
/// a velox process. Hence each query pool objects forms a subtree rooted
...
/// pointer protected by a mutex. The child pool object destruction first removes
...
/// also provides  memory usage accounting through MemoryUsageTracker. This
/// will be merged into memory pool object later.
///
  // oerling: Comment no longer valid?
...
class MemoryPool : public std::enable_shared_from_this<MemoryPool> {
 public:
  ...
  /// Returns the list of supported size class sizes used by non-contiguous memory
  /// allocation.
  ...
  // oerling: Can we call this tracker()? New getters should just have the member name and no get, unless they actually do something.
  ...
  /// Used for external aggregation.  oerling: can you clarify ?
  virtual void setSubtreeMemoryUsage(int64_t size) = 0;

  // oerling: Comment?
  virtual int64_t updateSubtreeMemoryUsage(int64_t size) = 0;

  /// Used to manage existing externally allocated memory without doing a new
  /// allocation. oerling: Document effect on tracker.
  virtual void reserve(int64_t /* bytes */) {
    VELOX_NYI("reserve() needs to be implemented in derived memory pool.");
  }

  // oerling: How does this relate to reserve? 'management' is better than 'governance' in this context.
 ...
 protected:
  ...
  /// Invoked only on destruction to remove this memory pool from its
  ...
  //oerling: Is this used? If used outside of the file, should not be in ::detail?
  namespace detail {
...
class MemoryPoolImpl : public MemoryPool {
 public:
  ...
  // oerling: delete commented out code?
  // oerling: Formatting: Do not use full rows of  or -- or == for framing things.
...
oerling: addChild or newChild would be better since get usually means accessing existing one.
  ...
  // oerling: The below is notr entirely clear. 
...
class MemoryManager final : public IMemoryManager {
 public:
  ...
  //oerling: comments for public functions?
